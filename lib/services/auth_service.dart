...
class AuthService {
  ...
  /// 🆕 Şifreli hesap silme — önce re-authenticate sonra sil
  static Future<void> deleteAccountWithPassword(String password) async {
    final user = currentUser;
    if (user == null) return;

    // Re-authenticate (Firebase Auth bunu şart koşuyor)
    ...
    // Re-auth başarılı, şimdi tüm verileri sil
    await deleteAccount();
  }
  ...
  static Future<void> deleteAccount() async {
    ...
    final friends =
        await _db.collection('friends').where('users', arrayContains: uid).get();
    ...
    final reqFrom =
        await _db.collection('friendRequests').where('from', isEqualTo: uid).get();
    for (final doc in reqFrom.docs) {
      await doc.reference.delete();
    }
    final reqTo =
        await _db.collection('friendRequests').where('to', isEqualTo: uid).get();
