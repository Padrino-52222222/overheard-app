...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  final List<String> _cities = [
    ...
    'Kocaeli', 'Sakarya', 'Muğla', 'Tekirdağ', 'Diğer',
  ];
  ...
  void _validateAndNext() {
    switch (_currentPage) {
      case 1:
        ...
        if (name.isEmpty) {
          _showWarning('İsim alanı boş. Devam etmek için bilgi girin veya atlayabilirsiniz.');
          return;
        }
        if (name.length < 2) {
      ...
      case 2:
        ...
        if (ageText.isEmpty) {
          _showWarning('Yaş alanı boş. Devam etmek için bilgi girin veya atlayabilirsiniz.');
          return;
        }
      ...
      case 3:
        if (_selectedCity == null) {
          _showWarning('Şehir seçilmedi. Devam etmek için bilgi girin veya atlayabilirsiniz.');
          return;
        }
        break;
      case 4:
        if (_selectedGender == null) {
          _showWarning('Cinsiyet seçilmedi. Devam etmek için bilgi girin veya atlayabilirsiniz.');
          return;
        }
        break;
    }
  ...
  Future<void> _finish() async {
    ...
    try {
      ...
      // Preset avatar seçildiyse → Firebase'e yükle (eski silinir)
        ...
        photoUrl = await AvatarService.uploadPresetAvatar(_selectedPresetAvatar!);
      }
        ...
        children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: (_selectedPhoto == null && _selectedPresetAvatar == null)
                    ? LinearGradient(
                        colors: [
                          Colors.white.withOpacity(0.08),
                          Colors.white.withOpacity(0.03),
                    ...
                    : null,
                border: Border.all(
                  color: (_selectedPhoto != null || _selectedPresetAvatar != null)
                      ? const Color(0xFFFFD700).withOpacity(0.5)
                      : Colors.white.withOpacity(0.1),
                  width: 2,
                ),
                boxShadow: (_selectedPhoto != null || _selectedPresetAvatar != null)
                    ? [
                        BoxShadow(
