...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  // ── Yeni state'ler ──
  bool _bilgilerExpanded = true;
  bool _albumExpanded = true;
  List<String?> _albumPhotos = List.filled(6, null);
  ...
  Future<void> _loadAlbumPhotos() async {
    ...
    try {
      final doc = await FirebaseFirestore.instance
          ...
          .get();
      final data = doc.data();
      if (data != null && data['albumPhotos'] != null) {
        final list = List<String?>.from(data['albumPhotos']);
        while (list.length < 6) list.add(null);
        if (mounted) setState(() => _albumPhotos = list.take(6).toList());
      }
    } catch (_) {}
  }

  Future<void> _pickAlbumPhoto(int index) async {
    try {
      ...
      final ref = FirebaseStorage.instance
          .ref()
          .child('users/${user.uid}/album/$index.jpg');
      ...
      final newPhotos = List<String?>.from(_albumPhotos);
      newPhotos[index] = url;

      await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .update({'albumPhotos': newPhotos});

      if (mounted) {
        setState(() {
          _albumPhotos = newPhotos;
          _albumLoading = false;
        });
      }
    } catch (e) {
                    ...
                    children: [
                      Expanded(
                        child: GestureDetector(
                          onTap: isDeleting ? null : () => Navigator.pop(ctx),
                          child: Container(
                            height: 48,
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(14),
                        ...
                        child: GestureDetector(
                          onTap: isDeleting
                              ...
                              : () async {
                                  ...
                                  if (pass.isEmpty) {
                                    setSheetState(() =>
                                        passError = 'Şifre boş bırakılamaz');
                                    return;
                                  }
                                  setSheetState(() {
                                    passError = null;
                                  ...
                                  try {
                                    await AuthService.deleteAccountWithPassword(
                                        pass);
                                    if (mounted) Navigator.pop(ctx);
                                  } on FirebaseAuthException catch (e) {
                                    String msg;
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _isDeletingAccount ? 'Hesap siliniyor...' : '',
                  style: TextStyle(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: const Color(0xFF9B30FF).withOpacity(0.3),
                            blurRadius: 20,
              ...
              // ── Albüm (açılır/kapanır) ──
              _buildCollapsibleHeader(
                title: '📷 Albüm',
                isExpanded: _albumExpanded,
                onToggle: () =>
                    setState(() => _albumExpanded = !_albumExpanded),
              ),
              AnimatedSize(
                duration: const Duration(milliseconds: 280),
                curve: Curves.easeInOut,
                child: _albumExpanded
                    ? Padding(
              ...
              // ── Bilgiler (açılır/kapanır) ──
              _buildCollapsibleHeader(
                title: '📋 Bilgiler',
                isExpanded: _bilgilerExpanded,
                onToggle: () =>
                    setState(() => _bilgilerExpanded = !_bilgilerExpanded),
              ),
              AnimatedSize(
                duration: const Duration(milliseconds: 280),
                curve: Curves.easeInOut,
                child: _bilgilerExpanded
                    ? Column(
                        children: [
                          const SizedBox(height: 12),
                          _buildInfoCard(
  ...
  Widget _buildKarizmaCard() {
    ...
    final levelColors = KarizmaService.getLevelColors(_karizma);
    final isLegendary = _karizma >= 1000;
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: LinearGradient(
              ...
              colors: [
                levelColors.first.withOpacity(0.15),
                const Color(0xFF0F0A1F),
              ],
            ),
            border: Border.all(
              color: levelColors.first.withOpacity(0.3),
              width: 1,
            ),
            boxShadow: [
              BoxShadow(
                color: levelColors.first.withOpacity(0.12),
                blurRadius: 20,
          ...
          child: Column(
            children: [
                ...
                children: [
                  ShaderMask(
                    shaderCallback: (bounds) =>
                        LinearGradient(colors: levelColors)
                            .createShader(bounds),
                    child: const Icon(
              ...
              ShaderMask(
                shaderCallback: (bounds) =>
                    LinearGradient(colors: levelColors).createShader(bounds),
                child: Text(
              ...
              Stack(
                children: [
                  Container(
                    height: isLegendary ? 10 : 8,
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(isLegendary ? 5 : 4),
                      color: Colors.white.withOpacity(0.06),
                    ),
                  ),
                  FractionallySizedBox(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        borderRadius:
                            BorderRadius.circular(isLegendary ? 5 : 4),
                        gradient: LinearGradient(colors: levelColors),
                        boxShadow: isLegendary
                            ? [
                                BoxShadow(
                                  color: levelColors.first.withOpacity(0.6),
                                  blurRadius: 8,
                                  spreadRadius: 1,
                                ),
                              ]
              ...
              Row(
                ...
                children: [
                  Text('${(animProgress * 100).toInt()}%',
                      style: TextStyle(
                          color: levelColors.first.withOpacity(0.7),
                          fontSize: 11,
                          fontWeight: FontWeight.w700)),
                  Text(
                      _karizma >= 5000
...
// ══════════════════════════════════════
  Widget _buildAlbumGrid() {
    return GridView.builder(
      ...
      itemCount: 6,
      itemBuilder: (ctx, i) {
        final url = _albumPhotos[i];
        return GestureDetector(
          onTap: () {
            if (url != null) {
              _showAlbumPhotoOptions(i, url);
            } else {
              _pickAlbumPhoto(i);
            }
          },
...
// ── Foto seçenekleri (değiştir / sil / büyüt) ──
  void _showAlbumPhotoOptions(int index, String url) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
      builder: (ctx) => Container(
...
// ── Foto silme ──
  Future<void> _deleteAlbumPhoto(int index) async {
    try {
      setState(() => _albumLoading = true);
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return;
    ...
    Navigator.of(context).push(
      PageRouteBuilder(
        ...
        pageBuilder: (_, animation, __) => FadeTransition(
          opacity: animation,
          child: _FullPhotoViewer(url: url),
        ),
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Column(
          children: [
            Icon(icon,
                color: isSelected ? Colors.white : Colors.white38, size: 28),
            const SizedBox(height: 6),
            Text(label,
                style: TextStyle(
...
// ══════════════════════════════════════
//  Full Photo Viewer (animasyonlu büyütme)
// ══════════════════════════════════════
class _FullPhotoViewer extends StatefulWidget {
  final String url;
  const _FullPhotoViewer({required this.url});
